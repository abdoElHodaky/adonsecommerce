...
export default class ViewMiddleware {
  ...
  private addRequestGlobals(ctx: HttpContext) {
    ...
    ctx["view"].global('formatCurrency', async (amount: number) => `$${amount.toFixed(2)}`)
    ctx["view"].global('formatDate', async (date: string | Date) => new Date(date).toLocaleDateString())
    ctx["view"].global('calculateDiscount', async (original: number, sale: number) => Math.round((1 - sale / original) * 100))
    ctx["view"].global('truncate', async (text: string, length = 100) => text.length > length ? text.substring(0, length) + '...' : text)
    ctx["view"].global('currentYear', async () => new Date().getFullYear()) 
